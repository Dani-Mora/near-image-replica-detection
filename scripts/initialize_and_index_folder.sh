...
# Description: This job initializes the detector and indexes the images from the input folder.
# It assumes that the dentifier of the image is given by its name. So ALL FILES MUST HAVE A NUMERIC NAME
...
$SPARK_BIN/spark-submit --class org.twitterReplica.jobs.IndexingFolderJobReset --master $SPARK_MASTER --driver-library-path $OPENCV_PATH target/replica-0.0.1-SNAPSHOT-jar-with-dependencies.jar $DESCRIPTOR_TYPE $KEYPOINT_TYPE $SIDE $FEATURE_FILTERING $FEATURE_THRESH $LOG_SCALE $NUM_TABLES $W $HAMMING_THRESH $ENCODING $COMPRESSION $TTL $PERSISTENCE $FOLDER $MEM_FILE $HBASE_MASTER $ZOOKEEPER_PORT $ZOOKEEPER_HOST $PARTITIONS
