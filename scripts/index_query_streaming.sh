...
# Description: Queries images received by the Twitter Streaming API and indexed them afterwards
...
$SPARK_BIN/spark-submit --class org.twitterReplica.jobs.streaming.IndexAndQueryStreamingJob --master $SPARK_MASTER --jars $TWITTER4J_CORE,$TWITTER4J_STREAMING --driver-library-path $OPENCV_PATH target/replica-0.0.1-SNAPSHOT-jar-with-dependencies.jar $DESCRIPTOR_TYPE $KEYPOINT_TYPE $SIDE $FEATURE_FILTERING $FEATURE_THRESH $LOG_SCALE $NUM_TABLES $W $HAMMING_THRESH $ENCODING $COMPRESSION $TTL $PERSISTENCE $DURATION $PER_SECOND $MEM_FILE $HBASE_MASTER $ZOOKEEPER_PORT $ZOOKEEPER_HOST $CONSUMER_KEY $CONSUMER_SECRET $ACCESS_TOKEN $ACCESS_TOKEN_SECRET $WEIGHT_FILT
