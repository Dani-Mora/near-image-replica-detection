...
# Description: This job evaluates the UPCReplica dataset for the given parameters 
...
PERSISTENCE=0 # Persistence mode. Set to 0 if you want to index and query files on memory and 1 for disk (HBase).
...
DATASET_QUERY_FILE=/path/to/query.csv # An example of this file can be found in github
DEST_FILE=/path/to/results # Destination file of the results
...
$SPARK_BIN/spark-submit --class org.twitterReplica.jobs.EvaluationJob --master $SPARK_MASTER --driver-library-path $OPENCV_PATH target/replica-0.0.1-SNAPSHOT-jar-with-dependencies.jar $DESCRIPTOR_TYPE $KEYPOINT_TYPE $SIDE $FEATURE_FILTERING $FEATURE_THRESH $LOG_SCALE $NUM_TABLES $W $HAMMING_THRESH $ENCODING $COMPRESSION $TTL $PERSISTENCE $WEIGHT_FILT $DATASET_PATH $DATASET_SRC_FILE $DATASET_QUERY_FILE $DEST_FILE $MEM_FILE $HBASE_MASTER $ZOOKEEPER_PORT $ZOOKEEPER_HOST $PARTITIONS
