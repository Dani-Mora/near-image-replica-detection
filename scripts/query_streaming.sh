...
# Description: Queries images received by the Twitter Streaming API
...
WEIGHT_FILT=2 # Weight filtering threshold
...
$SPARK_BIN/spark-submit --class org.twitterReplica.jobs.streaming.QueryStreamingJob --master $SPARK_MASTER --jars $TWITTER4J_CORE,$TWITTER4J_STREAMING --driver-library-path $OPENCV_PATH target/replica-0.0.1-SNAPSHOT-jar-with-dependencies.jar $DURATION $WEIGHT_FILT $PERSISTENCE $PER_SECOND $MEM_FILE $HBASE_MASTER $ZOOKEEPER_PORT $ZOOKEEPER_HOST $CONSUMER_KEY $CONSUMER_SECRET $ACCESS_TOKEN $ACCESS_TOKEN_SECRET
